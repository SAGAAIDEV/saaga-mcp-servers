...
3. Verify the scaffolding works by testing the included echo server:
   ...
   mcp_gemini-server --transport sse --port 3001 &  # Start server in background
...
```
mcp_gemini/              # Project Root
├── mcp_gemini/          # Python package directory
...
- `mcp_gemini/mcp_gemini/server/app.py`: Core MCP server implementation with unified transport handling and tool registration. This is the main server application module.
- `mcp_gemini/mcp_gemini/tools/`: Directory containing individual tool implementations (e.g., `echo.py`).
- `mcp_gemini/mcp_gemini/client/app.py`: Convenience client application for testing your MCP server.
...
Your MCP server uses a single entry point, `mcp_gemini-server`, and supports two transport modes, selectable via the `--transport` flag.
...
- **Invocation**: 
  - When you run `mcp_gemini-client`, it automatically starts and communicates with `mcp_gemini-server` in stdio mode.
  - To run the server directly in stdio mode (e.g., for testing with `mcp-cli` or other tools that manage the process):
    ```bash
    mcp_gemini-server --transport stdio
